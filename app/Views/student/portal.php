...
<body class="bg-slate-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto p-8 mt-4">
        ...
        <div class="mt-8 bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
            <h2 class="text-xl font-bold text-slate-800 mb-6">My Profile</h2>
            <?php if(isset($_GET['profile']) && $_GET['profile'] === 'updated'): ?>
                <p class="text-emerald-600 font-bold mb-4">Profile updated successfully!</p>
            <?php endif; ?>
            <form action="/student/profile/update" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
                
                <div class="flex items-center gap-6">
                    <div class="w-24 h-24 rounded-full bg-slate-200 border-4 border-white shadow-lg overflow-hidden shrink-0">
                        <?php if(!empty($studentData['profile_image'])): ?>
                            <img src="<?php echo htmlspecialchars($studentData['profile_image']); ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Change Avatar</label>
                        <input type="file" name="avatar" accept="image/*" class="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="phone" value="<?php echo htmlspecialchars($studentData['phone'] ?? ''); ?>" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1">Residential Address</label>
                        <textarea name="address" rows="3" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-500"><?php echo htmlspecialchars($studentData['address'] ?? ''); ?></textarea>
                    </div>
                    <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition-all">Save Profile</button>
                </div>
            </form>
        </div>
    </div>
